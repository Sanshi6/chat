...
def is_aiproxy_api_key(key):
    API_MATCH_ORIGINAL = re.match(r"ap-[a-zA-Z0-9]{48}$", key)
    API_MATCH_AZURE = re.match(r"[a-zA-Z0-9]{32}$", key)
    return bool(API_MATCH_ORIGINAL) or bool(API_MATCH_AZURE)
...
def is_any_api_key(key):
    if ',' in key:
        ...
        return is_openai_api_key(key) or is_api2d_key(key) or is_aiproxy_api_key(key)
...
def select_api_key(keys, llm_model):
    ...
    if llm_model.startswith('gpt-'):
        for k in key_list:
            ...
            if is_aiproxy_api_key(k): avail_key_list.append(k)
